...
@Pipe({
  name: 'filtroCcProveedores'
})
export class FiltroCcProveedoresPipe implements PipeTransform {
  ...
  transform(valores: any[], parametro: string, activo: string): any {
    ...
    if(parametro.length !== 0){
      return filtrados.filter( valor => { 
        return valor.proveedor.descripcion.toLocaleLowerCase().includes(parametro)
      });
    }else{
      return filtrados;
    }
